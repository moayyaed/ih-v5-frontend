...
class Tabs extends Component {
  ...
  componentDidMount() {
    ...
    context.events.on('app:navigator:click', (path, history, { label }) => {

      if (this.state.data.find(i => i.id === label)) {
        ...
        this.setState((state) => {
          return {
            ...state,
            select: label,
            data: state.data.concat({ id: label, label: label })
          }
        });
      }
    });
  ...
  componentWillUnmount() {
    context.events.on('app:navigator:exit', () => {
