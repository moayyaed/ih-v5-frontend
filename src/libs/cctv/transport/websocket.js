...
import { FWebSocket } from 'components/@Elements/@CCTV/transport';
...
export default class WebsocketTransport extends BaseTransport {
    constructor(endpoint, stream_type, options={
        socket:`${window.location.protocol.replace('http', 'ws')}//${window.location.host}/ws/`,
        workers: 1
    }) {
...
class WebSocketProxy {
    ...
    initDataChannel(channel_id) {
        return new Promise((resolve, reject)=>{
            this.dataChannel = new FWebSocket(this.url, WebSocketProxy.CHN_DATA);
    ...
    connect() {
        ...
        return new Promise((resolve, reject)=>{
            this.ctrlChannel = new FWebSocket(this.url, WebSocketProxy.CHN_CONTROL);
