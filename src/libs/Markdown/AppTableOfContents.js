...
const useStyles = makeStyles(theme => ({
  root: {
    top: core.isFrame ? 24 : 70,
    // Fix IE 11 position sticky issue.
    marginTop: core.isFrame ? 22 : 70,
    width: 175,
    flexShrink: 0,
    order: 2,
    position: 'sticky',
    height: core.isFrame ? '100vh' : 'calc(100vh - 70px)',
    overflowY: 'auto',
  ...
  renderer.heading = (text, level) => {
    if (level === 2) {
      ...
      if (!itemsCollector.current[itemsCollector.current.length - 1]) {
        ...
        const item = itemsCollector.current[itemsCollector.current.length - 1];
        if (item.level === 4) {
          itemsCollector.current.push({
            text,
            level: 4,
